...
#Using read.delim() will read the data based on the selected seperator, which in this case is a pipe symbol. Using file.choose() will let  you choose the file you want to use. This is better to use than the file path as if you use this code on a different computer, it does not matter where the file is saved.
#readr::guess_encoding(file.choose()) will help determine the fileEncoding which was done before writing it into the code below

sales_pipe = read.delim(file.choose(), fileEncoding = "ISO-8859-1", stringsAsFactors=FALSE, sep ="|")
```
...
# using the colnames() on the data in question and using box brackets will parse the data in the column number specified and  change it to thhe desired value
...
#find the structure of item, find if it is a date and then convert it based on the format (month, day, year in this case)
...
#find thhe structure of the item, find if it is a date and then convert it based on the format (month as a string, day,  yeaar in this case)
...
#find number of days b/w oldest and newest order using difftime. Set the units to the desired output of time
...
#Split the customer.name column into two segments, seperated by a space. Paste the first column of customer.name into a new column aas thhe first names and the same with the second column for the last names.
...
#find all the customers with first name Bill, then count how many there are with the length option
...
```{r}
# Your code here
#Use grep to pattern match the  word "table". Use the length operation to find hhow many occurances of the word there are
...
```{r}
# Your code here
#use the table operation to make a table of the states column. This will automatically be made in alphabetical order
...
```{r}
# Your code here
#use the barplot operation to make a chart of the category sales of Texas (specified  by the box brackets). This automatically plots in alphabetical order
...
```{r}
# Your code here
#Use the aggregate function to provide a summary statistic of the mean for the specified column. 
...
#split the order date and create a new column with just the order year. Split the Order.Date column into three segments, seperated by a "-". Paste the first segment of the split column into a new column named Order.Year
